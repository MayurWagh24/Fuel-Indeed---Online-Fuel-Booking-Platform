...
public class DeliveryPersonLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
		ps =con.prepareStatement("select * from deliveryperson where email=? and password=?");
		...
		{
			int dId=rs.getInt(1);
			getsetvalues.setId(dId);
			i= 1;
			System.out.println("Valid User");
		}
		if(i>0)
		{
			response.sendRedirect("3_DeliveryPersonDashboard.html");
		}
		else{
			response.sendRedirect("3_DeliveryPersonLogin.html");
		}
